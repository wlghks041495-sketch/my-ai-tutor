            ...
            with st.spinner("AI 선생님이 문장을 꼼꼼하게 분석 중입니다..."):
                for s in sentences:
                    ...
                    try:
                        ...
                        results.append({
                            'ko': s, 'en': ai_data.get('en', ''), 'zh': ai_data.get('zh', ''), 
                            'zh_pron': ai_data.get('zh_pron', ''), 'zh_desc': ai_data.get('zh_desc', ''),
                            'ja': ai_data.get('ja', ''), 'ja_pron': ai_data.get('ja_pron', ''), 'ja_desc': ai_data.get('ja_desc', '')
                        })
                    except Exception as e:
                        st.error(f"분석 오류: {e}")
